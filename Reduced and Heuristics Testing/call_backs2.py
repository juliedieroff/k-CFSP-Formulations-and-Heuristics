...
class Callback_lazy2(ConstraintCallbackMixin, LazyConstraintCallback):
    ...
    def __call__(self):
        ...
        if num_nodes == 0:
            ...
            self.root_time = time.time() - self.start_time
...
class HeuristicsCallback(ConstraintCallbackMixin, HeuristicCallback):
    ...
    def __call__(self):
       ...
       # Use the heuristic periodically at the root node
       if self.node_num == 0:
           if num_nodes == 0:
               self.node_zero_count += 1
               if self.node_zero_count == 1 or self.node_zero_count % self.frequency == 0:
                   # Get the current solution
                   sol_x = self.make_solution_from_vars(self.model_instance.x.values())
                   sol_y = self.make_solution_from_vars(self.model_instance.y.values())
                   ...
                   if self.heuristic_choice == 1:                   
                        ...
                        for i in self.problem_data.V:
                            if sol_y.get_value(self.model_instance.y[i,i]) > self.cutoff:
                                on_tree.append(i)
                        ...
                        T, S, self.cost = lp_heuristic(on_tree, sat_verts, self.problem_data.dist, self.problem_data)
                        ...
                        # Return the heuristic gap
                        lower_bound = self.get_objective_value()
                        temp = (self.cost - lower_bound)/self.cost
                        
                        if self.heuristic_gap is not None:
                            if temp < self.heuristic_gap:
                                self.heuristic_gap = temp
                        else:
                            self.heuristic_gap = temp
                        
                        # Set the heuristic solution in the solver
                        self.set_solution([variable, solution], self.cost) 
                   ...
                   if self.heuristic_choice == 2:
                       ...
                       T, S, self.cost, timeH = heuristic_sol(new_weights, self.problem_data)
                   ...
                   if self.heuristic_choice == 3:
                       ...
                       # Run the heuristic to find a feasible solution
                       T, S, self.cost = lp_heuristic(on_tree, sat_verts, new_weights, self.problem_data)
       ...
       # Uses the heuristic at nodes 1 and 2
       if self.node_num == 1:
